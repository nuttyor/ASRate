
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// ASRate
#define COCOAPODS_POD_AVAILABLE_ASRate
#define COCOAPODS_VERSION_MAJOR_ASRate 0
#define COCOAPODS_VERSION_MINOR_ASRate 1
#define COCOAPODS_VERSION_PATCH_ASRate 0

// Bolts/Tasks
#define COCOAPODS_POD_AVAILABLE_Bolts_Tasks
#define COCOAPODS_VERSION_MAJOR_Bolts_Tasks 1
#define COCOAPODS_VERSION_MINOR_Bolts_Tasks 2
#define COCOAPODS_VERSION_PATCH_Bolts_Tasks 0

// DQAlertView
#define COCOAPODS_POD_AVAILABLE_DQAlertView
#define COCOAPODS_VERSION_MAJOR_DQAlertView 1
#define COCOAPODS_VERSION_MINOR_DQAlertView 2
#define COCOAPODS_VERSION_PATCH_DQAlertView 1

// Parse
#define COCOAPODS_POD_AVAILABLE_Parse
#define COCOAPODS_VERSION_MAJOR_Parse 1
#define COCOAPODS_VERSION_MINOR_Parse 7
#define COCOAPODS_VERSION_PATCH_Parse 5

